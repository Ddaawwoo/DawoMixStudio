...
`;document.head.appendChild(s)}
  ...
  function boot(){mainStyle();brand();loadIconPack();loadCueTrackNav();setTimeout(loadWorkflow,80);setTimeout(loadSettingsCenter,120);setTimeout(loadDesktopIntegration,160);setTimeout(loadLibraryHealth,210);setTimeout(loadAutosave,250);setTimeout(loadCueUndo,290);setTimeout(loadTagUndo,320);setTimeout(loadAutoMatch,350);mainWave();cueFrame();playlistFrame();window.addEventListener('dawo-library-change',()=>setTimeout(()=>recolor(document.getElementById('analysisWaveform')),0))}

(() => {
  ...
  async function all(){const store=await txStore();const rows=await request(store.getAll());return rows.sort((a,b)=>(b.updatedAt||0)-(a.updatedAt||0));}
  async function get(uid){const store=await txStore();return request(store.get(uid));}
  ...
  async function remove(uid){const store=await txStore('readwrite');await request(store.delete(uid));window.dispatchEvent(new CustomEvent('dawo-library-change'));}
